...
def how_many_people(array)
  puts array.length
end

print "Nombre de journalistes : "
how_many_people(array_twitter)
...
def answer2(array)
  return "Le nombre de compte avec des chiffres est de #{array.grep(/[0-9]/).count}"
end

puts answer2(array_twitter)


#3 Combien d'handle contiennent les 4 lettres du prénom "Aude" à la suite (sans prendre en compte les majuscules) ?
def answer3(array)
  return "Le nombre de compte avec le prénom Aude dans leur pseudo est de #{array.grep(/aude(.*)/i).count}"
end

puts answer3(array_twitter)



#4 Combien commencent par une majuscule (première lettre juste après le @) ?
def answer4(array)
  return "Le nombre de compte commençant par une maj est de #{array.grep(/^@[A-Z]/).count}"
end

puts answer4(array_twitter)



#5 Combien contiennent au moins une majuscule ?
def answer5(array)
  return "Le nombre de compte commençant ayant une majuscule dans le pseudo est de #{array.grep(/[A-Z]/).count}"
end

puts answer5(array_twitter)
...
def answer6(array)
  return "Le nombre de compte commençant aynt un underscore dans le pseudo est de #{array.grep(/\_/).count}"
end

puts answer6(array_twitter)


# J'ai remarqué qu'il y a les maj avant les min, je regarderai plus tard
#7 Trie la liste de handle par ordre alphabétique.
def answer7(array)
  array_alpha = array.sort
  return array_alpha
end

# puts answer7(array_twitter)



#8 Quels sont les 50 handles les plus courts de ce array ?
def answer8(array)
  puts "Les 50 pseudos les plus courts sont :"
  twitter_sort = array.sort_by(&:length)
...
def answer9(array)
  print "La position dans l'array de la personne @epenser est : "
  print array.find_index('@epenser')
end


puts answer9(array_twitter)